...
class NeuralNetwork:
    ...
    def CreateNetwrok(self, gene: list):
        ...
        for i in range(len(gene)):
            ...
            for neuron_informations in layer:
        ...
        for i in range(len(gene)):
            for j in range(len(gene[i])): #for each neuron
                ...
                for k in range(2, len(neuron_informations) - 1): #for each link of that neuron
                    link = Link(self.search_neuron(neuron_informations[k]), weight=neuron_informations[k + 1]) #create the link

                    neuron.links.append(link) #append the link