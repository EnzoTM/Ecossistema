...
class NeuralNetwork: # fake natty
    ...
    def TrainDuringSimulation(self, new_position: tuple, old_position: tuple):
        #loss = math.sqrt(((new_position[0] - old_position[0])**2) + ((new_position[1] - old_position[1])**2))

        gradiente = (2*(new_position[0] - old_position[0]), 2*(new_position[1] - old_position[1]))
        gradiente = np.reshape(gradiente, (2, ))

        for layer in reversed(self.model):
            gradiente = layer.backward(gradiente, 0.1)
